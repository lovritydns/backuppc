...
$Lang{Admin_Options} = "Adminisztr▒tori opci▒k";
...
<table class="tableStnd">
  <tr><td>Szerver be▒ll▒t▒sainak ▒jrat▒lt▒se:<td><input type="button" value="▒jrat▒lt"
     onClick="document.ReloadForm.action.value='▒jrat▒lt';
...
$Lang{H_BackupPC_Server_Status} = "BackupPC Szerver St▒tusz";

$Lang{BackupPC_Server_Status_General_Info}= <<EOF;
\${h2(\"Szerver informc▒i▒k")}

<ul>
<li> A szerver PID-je: \$Info{pid}, a \$Conf{ServerHost} hoszton,
     Verzi▒: \$Info{Version}, ind▒tva: \$serverStartTime -kor.
<li> Ez a st▒tuszt gener▒lva: \$now -kor lett.
<li> Jelen konfigur▒ci▒ \$configLoadTime -kor lett bet▒ltve.
<li> PC utoljs▒ra \$nextWakeupTime lett elind▒tva.
<li> Egy▒b info::
    <ul>
        <li>\$numBgQueue v▒rakoz▒ ment▒s az utols▒ ▒temez▒s ▒ta,
        <li>\$numUserQueue v▒rakor▒ felhaszn▒l▒i ment▒s,
        <li>\$numCmdQueue v▒rakor▒  parancs van,
        \$poolInfo
...
\${h2("Figyelmet ig▒nyl▒ hib▒k")}
<p>
<table class="tableStnd" border cellspacing="1" cellpadding="3">
<tr class="tableheader"><td align="center"> Host </td>
    <td align="center"> T▒pus </td>
    <td align="center"> Felhaszn▒l▒ </td>
    <td align="center"> Last Try </td>
    <td align="center"> R▒szletek </td>
    <td align="center"> Hiba ideje </td>
    <td> Utols▒ hiba (kiv▒tel, ha nincs ping) </td></tr>
\$statusStr
</table>
EOF

# --------------------------------
$Lang{BackupPC__Server_Summary} = "BackupPC: Host ▒sszes▒t▒";
...
<p>
<ul>
<li>Ezt a st▒tuszt gener▒lva: \$now -kor lett.
<li>T▒rol▒ jelenlegi felhaszn▒l▒sa \$Info{DUlastValue}%
    (\$DUlastTime), a mai maximum  \$Info{DUDailyMax}% (\$DUmaxTime)
        ▒ a tegnapi maximum \$Info{DUDailyMaxPrev}%.
</ul>
</p>

\${h2("Hosztok, amelyeknek nincs biztons▒gi ment▒se")}
<p>
Jelenleg \$hostCntGood szerver van mentve, amelyek ▒sszesen::
<ul>
<li> \$fullTot teljes ment▒s, amelynek m▒rete \${fullSizeTot}GB
     (ment▒s ▒s t▒m▒r▒t▒s el▒tt),
<li> \$incrTot n▒vekm▒nyes ment▒s, melynek m▒rete: \${incrSizeTot}GB
     (ment▒s ▒s t▒m▒r▒t▒s el▒tt).
</ul>
</p>
...
<tr class="tableheader"><td> Szerver </td>
    ...
    <td align="center"> Teljes d▒tum (nap)</td>
    <td align="center"> Teljes m▒ret (GB) </td>
    <td align="center"> Sebess▒g (MB/s) </td>
    <td align="center"> #N▒vekm </td>
    <td align="center"> N▒vekm d▒tume (nap) </td>
    <td align="center"> Utols▒ ment▒s (nap) </td>
    <td align="center"> St▒tusz </td>
    <td align="center"> #Xfer hib▒k </td>
    <td align="center"> Utols▒ ▒tad▒s </td></tr>
\$strGood
</table>
<br><br>
...
# -----------------------------------
$Lang{Pool_Stat} = <<EOF;
        <li>A t▒ol▒ eddigi m▒rete \${poolSize} GB, ami \$info->{"\${name}FileCnt"} file-t
            ▒s  \$info->{"\${name}DirCnt"} k▒nyvt▒rat tartalmaz.(Lek▒rdezve: \$poolTime),
        <li>A t▒rol▒ \$info->{"\${name}FileCntRep"} ism▒telt file-t tartalmaz
            \$info->{"\${name}FileRepMax"} l▒ncon,
        <li>Az ▒jszakai tiszt▒t▒s eltv▒ol▒tott \$info->{"\${name}FileCntRm"} file-t, ami
            \${poolRmSize} GB m▒ret▒ volt (kb: \$poolTime -kor),
EOF
...
Utols▒ m▒dos▒t▒s a <tt>\$file</tt>, file-on \$mtimeStr -kor. \$comment
EOF
...
# --------------------------------
$Lang{BackupPC__Log_File_History} = "BackupPC: Log file t▒rt▒netek";
...
# ------------------------------
$Lang{Browse_backup__num_for__host} = "BackupPC: Ment▒s tall▒z▒sa \$num for \$host";
...
$Lang{backupType_incr}    = "n▒vekm";
$Lang{backupType_partial} = "r▒szleges";
...
$Lang{Status_idle} = "t▒tlen";
$Lang{Status_backup_starting} = "ment▒s indul";
$Lang{Status_backup_in_progress} = "ment▒s folyamatban";
$Lang{Status_restore_starting} = "vissza▒ll▒t▒s indul";
$Lang{Status_restore_in_progress} = "vissza▒ll▒t▒s folyamatban";
$Lang{Status_link_pending} = "f▒gg▒ben l▒v▒";
$Lang{Status_link_running} = "link fut";

$Lang{Reason_backup_done}    = "k▒sz";
$Lang{Reason_restore_done}   = "vissza▒ll▒t▒s k▒sz";
$Lang{Reason_archive_done}   = "ment▒s k▒sz";
$Lang{Reason_nothing_to_do}  = "t▒tlen";
$Lang{Reason_backup_failed}  = "ment▒si hiba";
$Lang{Reason_restore_failed} = "vissza▒ll▒t▒si hiba";
$Lang{Reason_archive_failed} = "ment▒s hiba";
$Lang{Reason_no_ping}        = "nincs ping";
$Lang{Reason_backup_canceled_by_user}  = "ment▒st felhaszn▒l▒ le▒ll▒tva";
$Lang{Reason_restore_canceled_by_user} = "restore canceled by user";
$Lang{Reason_archive_canceled_by_user} = "archive felhaszn▒l▒ le▒ll▒tva";
...
$Lang{CfgEdit_Edit_Config} = "Be▒l▒t▒sok";
$Lang{CfgEdit_Edit_Hosts}  = "Szerverek";

$Lang{CfgEdit_Title_Server} = "Szerver";
$Lang{CfgEdit_Title_General_Parameters} = "Rendszer param▒terek";
...
$Lang{CfgEdit_Title_Xfer_Settings} = "Xfer be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Ftp_Settings} = "FTP be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Smb_Settings} = "Smb be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Tar_Settings} = "Tar be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Rsync_Settings} = "Rsync be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Rsyncd_Settings} = "Rsyncd be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Archive_Settings} = "Archive be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Include_Exclude} = "Include/Exclude";
$Lang{CfgEdit_Title_Smb_Paths_Commands} = "Smb ▒tvonal/parancs";
$Lang{CfgEdit_Title_Tar_Paths_Commands} = "Tar ▒tvonal/parancs";
$Lang{CfgEdit_Title_Rsync_Paths_Commands_Args} = "Rsync ▒tvonal/parancs/param▒terek";
$Lang{CfgEdit_Title_Rsyncd_Port_Args} = "Rsyncd Port/Args";
$Lang{CfgEdit_Title_Archive_Paths_Commands} = "Archive Paths/Commands";
$Lang{CfgEdit_Title_Schedule} = "▒temez▒s";
$Lang{CfgEdit_Title_Full_Backups} = "Teljes ment▒sek";
$Lang{CfgEdit_Title_Incremental_Backups} = "N▒vekm▒nyes ment▒sek";
$Lang{CfgEdit_Title_Blackouts} = "Kimarad▒s";
$Lang{CfgEdit_Title_Other} = "Egy▒b";
$Lang{CfgEdit_Title_Backup_Settings} = "Ment▒si be▒ll▒t▒sok";
$Lang{CfgEdit_Title_Client_Lookup} = "Client Lookup";
$Lang{CfgEdit_Title_User_Commands} = "Felhaszn▒l▒i parancsok";
...
\${h1("Rendszerbe▒ll▒t▒sok szerkeszt▒se")}
EOF

$Lang{CfgEdit_Header_Host} = <<EOF;
\${h1("Host \$host be▒ll▒t▒sok szerkeszt▒se")}
<p>
Note: Check Override if you want to modify a value specific to this host.
<p>
EOF

$Lang{CfgEdit_Button_Save}     = "Ment▒s";
$Lang{CfgEdit_Button_Insert}   = "Besz▒r";
$Lang{CfgEdit_Button_Delete}   = "T▒r▒l";
$Lang{CfgEdit_Button_Add}      = "Hozz▒ad";
$Lang{CfgEdit_Button_Override} = "Fel▒l▒r";
$Lang{CfgEdit_Button_New_Key}  = "▒j kulcs";
$Lang{CfgEdit_Button_New_Share} = "▒ megoszt▒s vagy '*'";
