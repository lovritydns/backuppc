...
$Lang{Start_Archive} = "Mentes keszitese";
$Lang{Stop_Dequeue_Archive} = "Stop/Dequeue Archive";
$Lang{Start_Full_Backup} = "Teljes mentes inditasa";
$Lang{Start_Incr_Backup} = "Inkl. mentes inditasa";
$Lang{Stop_Dequeue_Backup} = "Stop/Dequeue Backup";
$Lang{Restore} = "Restore";

$Lang{Type_full} = "teljes";
$Lang{Type_incr} = "inkrementál";

# -----

$Lang{Only_privileged_users_can_view_admin_options} = "Only privileged users can view admin options.";
$Lang{H_Admin_Options} = "BackupPC Szerver: Admin Opciok";
$Lang{Admin_Options} = "Adminisztrátori opciók";
...
<table class="tableStnd">
  <tr><td>Szerver beállításainak újratöltése:<td><input type="button" value="Újratölt"
     onClick="document.ReloadForm.action.value='Újratölt';
...
<ul>
  ...
  <li>Szerver konfig szerkesztese
</ul>
-->
EOF

$Lang{Unable_to_connect_to_BackupPC_server} = "Nem tudtam csatlakozni a BackupPc szerverhez";
...
$Lang{H_BackupPC_Server_Status} = "BackupPC Szerver Státusz";

$Lang{BackupPC_Server_Status_General_Info}= <<EOF;
\${h2(\"Szerver informcáiók")}

<ul>
<li> A szerver PID-je: \$Info{pid}, a \$Conf{ServerHost} hoszton,
     Verzió: \$Info{Version}, indítva: \$serverStartTime -kor.
<li> Ez a státuszt generálva: \$now -kor lett.
<li> Jelen konfiguráció \$configLoadTime -kor lett betöltve.
<li> PC utoljsára \$nextWakeupTime lett elindítva.
<li> Egyéb info::
    <ul>
        <li>\$numBgQueue várakozó mentés az utolsó ütemezés óta,
        <li>\$numUserQueue várakoró felhasználói mentés,
        <li>\$numCmdQueue várakoró  parancs van,
        \$poolInfo
...
\${h2("Figyelmet igénylö hibák")}
<p>
<table class="tableStnd" border cellspacing="1" cellpadding="3">
<tr class="tableheader"><td align="center"> Host </td>
    <td align="center"> Típus </td>
    <td align="center"> Felhasználó </td>
    <td align="center"> Last Try </td>
    <td align="center"> Részletek </td>
    <td align="center"> Hiba ideje </td>
    <td> Utolsó hiba (kivétel, ha nincs ping) </td></tr>
\$statusStr
</table>
EOF

# --------------------------------
$Lang{BackupPC__Server_Summary} = "BackupPC: Host összesítö";
...
<p>
<ul>
<li>Ezt a státuszt generálva: \$now -kor lett.
<li>Tároló jelenlegi felhasználása \$Info{DUlastValue}%
    (\$DUlastTime), a mai maximum  \$Info{DUDailyMax}% (\$DUmaxTime)
        é a tegnapi maximum \$Info{DUDailyMaxPrev}%.
</ul>
</p>

\${h2("Hosztok, amelyeknek nincs biztonsági mentése")}
<p>
Jelenleg \$hostCntGood szerver van mentve, amelyek összesen::
<ul>
<li> \$fullTot teljes mentés, amelynek mérete \${fullSizeTot}GB
     (mentés és tömörítés elött),
<li> \$incrTot növekményes mentés, melynek mérete: \${incrSizeTot}GB
     (mentés és tömörítés elött).
</ul>
</p>
<table class="sortable" id="host_summary_backups" border cellpadding="3" cellspacing="1">
<tr class="tableheader"><td> Szerver </td>
    <td align="center"> Felhasználó </td>
    <td align="center"> #Teljes </td>
    <td align="center"> Teljes dátum (nap)</td>
    <td align="center"> Teljes méret (GB) </td>
    <td align="center"> Sebesség (MB/s) </td>
    <td align="center"> #Növekm </td>
    <td align="center"> Növekm dátume (nap) </td>
    <td align="center"> Utolsó mentés (nap) </td>
    <td align="center"> Státusz </td>
    <td align="center"> #Xfer hibák </td>
    <td align="center"> Utolsó átadás </td></tr>
\$strGood
</table>
<br><br>
...
# -----------------------------------
$Lang{Pool_Stat} = <<EOF;
        <li>A táoló eddigi mérete \${poolSize} GB, ami \$info->{"\${name}FileCnt"} file-t
            és  \$info->{"\${name}DirCnt"} könyvtárat tartalmaz.(Lekérdezve: \$poolTime),
        <li>A tároló \$info->{"\${name}FileCntRep"} ismételt file-t tartalmaz 
            \$info->{"\${name}FileRepMax"} láncon,
        <li>Az éjszakai tisztítás eltváolított \$info->{"\${name}FileCntRm"} file-t, ami 
            \${poolRmSize} GB méretü volt (kb: \$poolTime -kor),
EOF
...
# --------------------------------
$Lang{BackupPC__Queue_Summary} = "BackupPC: Queue osszesito";
# --------------------------------
$Lang{Backup_Queue_Summary} = <<EOF;
\${h1("Backup Queue osszesito")}
<br><br>
\${h2("User Queue osszesito")}
<p>
...
Utolsó módosítás a <tt>\$file</tt>, file-on \$mtimeStr -kor. \$comment
EOF

# --------------------------------
$Lang{skipped__skipped_lines} = "[ skipped \$skipped lines ]\n";
# --------------------------------
$Lang{_pre___Can_t_open_log_file__file} = "<pre>\nNem lehet a \$file log file-t megnyitni.\n";

# --------------------------------
$Lang{BackupPC__Log_File_History} = "BackupPC: Log file történetek";
...
# ------------------------------
$Lang{Browse_backup__num_for__host} = "BackupPC: Mentâ–’s tallâ–’zâ–’sa \$num for \$host";
...
$Lang{Status} = "Státusz";
$Lang{PC_Summary} = "Összesítö";
$Lang{LOG_file} = "LOG napló";
$Lang{LOG_files} = "LOG files";
$Lang{Old_LOGs} = "Régebbi LOG-ok";
$Lang{Email_summary} = "Email összesítö	";
$Lang{Config_file} = "Config file";
$Lang{Hosts_file} = "Hosts file";
$Lang{Current_queues} = "Jelenleg folyamatok";
$Lang{Documentation} = "Dokumentáció";

#$Lang{Host_or_User_name} = "<small>Host or User name:</small>";
$Lang{Go} = "Ugrás";
$Lang{Hosts} = "Hosts";
$Lang{Select_a_host} = "Válasszon...";
...
$Lang{backupType_full}    = "teljes";
$Lang{backupType_incr}    = "növekm";
$Lang{backupType_partial} = "részleges";
...
$Lang{Status_idle} = "tétlen";
$Lang{Status_backup_starting} = "mentés indul";
$Lang{Status_backup_in_progress} = "mentés folyamatban";
$Lang{Status_restore_starting} = "visszaállítás indul";
$Lang{Status_restore_in_progress} = "visszaállítás folyamatban";
$Lang{Status_link_pending} = "függöben lévö";
$Lang{Status_link_running} = "link fut";

$Lang{Reason_backup_done}    = "kész";
$Lang{Reason_restore_done}   = "visszaállítás kész";
$Lang{Reason_archive_done}   = "mentés kész";
$Lang{Reason_nothing_to_do}  = "tétlen";
$Lang{Reason_backup_failed}  = "mentési hiba";
$Lang{Reason_restore_failed} = "visszaállítási hiba";
$Lang{Reason_archive_failed} = "mentés hiba";
$Lang{Reason_no_ping}        = "nincs ping";
$Lang{Reason_backup_canceled_by_user}  = "mentést felhasználó leállítva";
$Lang{Reason_restore_canceled_by_user} = "restore canceled by user";
$Lang{Reason_archive_canceled_by_user} = "archive felhasználó leállítva";
$Lang{Disabled_OnlyManualBackups}  = "auto tiltva";
$Lang{Disabled_AllBackupsDisabled} = "tiltva";
...
Ez a szerver ($host) has never been successfully backed up by our
...
$Lang{CfgEdit_Edit_Config} = "Beálítások";
$Lang{CfgEdit_Edit_Hosts}  = "Szerverek";

$Lang{CfgEdit_Title_Server} = "Szerver";
$Lang{CfgEdit_Title_General_Parameters} = "Rendszer paraméterek";
$Lang{CfgEdit_Title_Wakeup_Schedule} = "Wakeup Schedule";
$Lang{CfgEdit_Title_Concurrent_Jobs} = "Konkurens Jobs";
...
$Lang{CfgEdit_Title_Xfer_Settings} = "Xfer beállítások";
$Lang{CfgEdit_Title_Ftp_Settings} = "FTP beállítások";
$Lang{CfgEdit_Title_Smb_Settings} = "Smb beállítások";
$Lang{CfgEdit_Title_Tar_Settings} = "Tar beállítások";
$Lang{CfgEdit_Title_Rsync_Settings} = "Rsync beállítások";
$Lang{CfgEdit_Title_Rsyncd_Settings} = "Rsyncd beállítások";
$Lang{CfgEdit_Title_Archive_Settings} = "Archive beállítások";
$Lang{CfgEdit_Title_Include_Exclude} = "Include/Exclude";
$Lang{CfgEdit_Title_Smb_Paths_Commands} = "Smb útvonal/parancs";
$Lang{CfgEdit_Title_Tar_Paths_Commands} = "Tar útvonal/parancs";
$Lang{CfgEdit_Title_Rsync_Paths_Commands_Args} = "Rsync útvonal/parancs/paraméterek";
$Lang{CfgEdit_Title_Rsyncd_Port_Args} = "Rsyncd Port/Args";
$Lang{CfgEdit_Title_Archive_Paths_Commands} = "Archive Paths/Commands";
$Lang{CfgEdit_Title_Schedule} = "Ütemezés";
$Lang{CfgEdit_Title_Full_Backups} = "Teljes mentések";
$Lang{CfgEdit_Title_Incremental_Backups} = "Növekményes mentések";
$Lang{CfgEdit_Title_Blackouts} = "Kimaradás";
$Lang{CfgEdit_Title_Other} = "Egyéb";
$Lang{CfgEdit_Title_Backup_Settings} = "Mentési beállítások";
$Lang{CfgEdit_Title_Client_Lookup} = "Client Lookup";
$Lang{CfgEdit_Title_User_Commands} = "Felhasználói parancsok";
$Lang{CfgEdit_Title_Hosts} = "Hosztok";
...
\${h1("Rendszerbeállítások szerkesztése")}
EOF

$Lang{CfgEdit_Header_Host} = <<EOF;
\${h1("Host \$host beállítások szerkesztése")}
<p>
Note: Check Override if you want to modify a value specific to this host.
<p>
EOF

$Lang{CfgEdit_Button_Save}     = "Mentés";
$Lang{CfgEdit_Button_Insert}   = "Beszúr";
$Lang{CfgEdit_Button_Delete}   = "Töröl";
$Lang{CfgEdit_Button_Add}      = "Hozzáad";
$Lang{CfgEdit_Button_Override} = "Felülír";
$Lang{CfgEdit_Button_New_Key}  = "Új kulcs";
$Lang{CfgEdit_Button_New_Share} = "Ú megosztás vagy '*'";
